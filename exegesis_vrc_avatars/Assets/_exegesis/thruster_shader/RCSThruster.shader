...
Shader "exegesis/RCSThruster"
{
    Properties
    {
        ...
        // These are how much authority a thruster keeps in the OTHER job.
        //
        // DEFAULT 1 = no split, deliberately. At 0 a thruster loses the other job
        // entirely, so shipping 0 meant an unpainted avatar (blue 0 everywhere) had its
        // torque term multiplied by zero on every thruster and rotation stopped working
        // avatar-wide. An added feature must not silently remove existing behaviour.
        // Lower these toward 0 AFTER the blue paint exists, to enable the split.
        _RotThrusterLinGain ("Rotation Thruster   Linear Keep", Range(0, 1)) = 1
        _TransThrusterRotGain ("Translation Thruster   Rotation Keep", Range(0, 1)) = 1
        ...
        [Header(Debug)]
        [Enum(Off, 0, ThrustDirection, 1, Throttle, 2, Groups, 3, Factors, 4, RotationBias, 5, RawGreenCalib, 6)] _DebugView ("Debug View", Float) = 0
    }
