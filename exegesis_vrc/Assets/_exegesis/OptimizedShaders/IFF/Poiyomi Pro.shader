Shader "Hidden/Locked/.poiyomi/Poiyomi Pro/a68b599db2e68424ebb6aec34c73bf4d"
{
	Properties
	{
		...
		[HideInInspector][ThryWideEnum(UV0, 0, UV1, 1, UV2, 2, UV3, 3, Panosphere, 4, World Pos, 5, Local Pos, 8, Polar UV, 6, Distorted UV, 7, Matcap, 9)] _MirrorTextureUV ("UV", Int) = 0
		...
		[HideInInspector] m_start_vertexLookAt (" LookAt--{reference_property:_VertexLookAtEnabled, button_help:{text:Tutorial,action:{type:URL,data:https://www.poiyomi.com/vertex-options/look-at},hover:Documentation}}}", Float) = 0
 ...
 #define OPTIMIZER_ENABLED 
			...
			VertexOut vert(appdata v)
			{
				...
				#if defined(PROP_LOOKATMASK) || !defined(OPTIMIZER_ENABLED)
				lookAtMask = _LookAtMask.SampleLevel(sampler_linear_repeat, poiUV(vertexUV(v, 0.0), float4(1,1,0,0)) + _Time.x * float4(0,0,0,0), 0);
				#endif
				lookAtMask *= alpha;
				...
				if (1.0)
				{
					...
					float4(0,0,0,0).xy,
					float4(0,0,0,0).xy,
					float4(-180,180,-180,180).xy,
					float4(-180,180,-180,180).xy,
					float4(-180,180,-180,180).xy
					);
					ApplyQuaternionRotation(v, qFinal, float4(0,0,0,1).xyz);
				}
				#endif
				#ifdef POI_MIRROR
				float notVisible = 0;
				if (1.0 == 1) // VRC
				{
					...
					notVisible += (!0.0 && (mirrorMode == 1));
					notVisible += (!0.0 && (mirrorMode == 2));
					notVisible += (!0.0 && (cameraMode == 1));
					notVisible += (!0.0 && (cameraMode == 2));
					notVisible += (!0.0 && (cameraMode == 3));
				}
			...
			void applyAlphaOptions(inout PoiFragData poiFragData, in PoiMesh poiMesh, in PoiCam poiCam, in PoiMods poiMods)
			{
				...
				if (1.0)
				{
					float3 position = 0.0 ? poiMesh.worldPos : poiMesh.objectPosition;
					float distanceFadeMultiplier = lerp(0.0, 1.0, smoothstep(3.0, 3.0, distance(position, poiCam.worldPos)));
					if (0.0 > 0)
					{
						distanceFadeMultiplier = lerp(1, distanceFadeMultiplier, poiMods.globalMask[0.0 - 1]);
					}
